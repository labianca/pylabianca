...
* API: removed stale `pylabianca.selectivity.compute_selectivity_windows()` in favor of selecting or averaging time windows before calling `pylabianca.selectivity.compute_selectivity_continuous()`.
...
* ENH: `pylabianca.utils.apply_dict()` for applying functions to all dictionary entries or trial selection / quering.
...
* ENH: `pylabianca.selectivity.compute_selectivity_continuous()` and `pylabianca.stats.permutation_test()` now accept `stat_fun='kruskal'` to use the nonparametric Kruskal-Wallis H statistic instead of the default t test / ANOVA F statistic.
